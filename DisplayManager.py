...
class DisplayManager(object):
    ...
    def __init__(self, name, isLog, ratio, xmin=0.22, ymin=0.6, xpos=None, ypos=None, label=None):
...
#        self.Legend = ROOT.TLegend(0.42, 0.6, 0.8, 0.85)
        ...
        self.draw_ratioLegend = ROOT.TLegend(0.15, 0.59, 0.5, 0.89)
...
#        self.canvas.Print(self.name + ']')
    ...
    def Draw(self, histos, titles):
        ...
        for i, h in enumerate(self.histos):
            ...
            if self.isLog:
                #h.GetYaxis().SetRangeUser(1., ymax * 10)
                #h.GetYaxis().SetRangeUser(0.00001, ymax * 100)
                h.GetYaxis().SetRangeUser(0.00001, 2)
                if ymax > 1:
                    h.GetYaxis().SetRangeUser(1, ymax*10)
